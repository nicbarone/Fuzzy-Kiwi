...
void GameplayMode::buildScene() {
    ...
    _enemyController->addEnemy(400, 0,  0, { 1 }, 400, 400, 5, enemyTexture, altTexture, enemyHighlightTexture);
    _enemyController->addEnemy(650, 0, 0, { 1 }, 300, 900, 5, enemyTexture, altTexture, enemyHighlightTexture);
    ...
    if (enemiesJSON != nullptr) {
        for (int i = 0; i < enemiesJSON->size(); i++) {
            ...
            _enemyController->addEnemy(objectTemp->getFloat("x_pos"), objectTemp->getInt("level"), 0, 
                key, objectTemp->getFloat("patrol_start"), objectTemp->getFloat("patrol_end"), 5, enemyTexture, altTexture, enemyHighlightTexture);
        }
    }
